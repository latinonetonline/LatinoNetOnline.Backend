...
{
    public record ConfirmProposalResponse(Guid ProposalId, string Title, string Description, DateTime EventDate, DateTime CreationTime, string? AudienceAnswer, string? KnowledgeAnswer, string? UseCaseAnswer, bool IsActive, int? WebinarNumber, WebinarStatus Status, Uri? Meetup, Uri? Streamyard, Uri? LiveStreaming, Uri? Flyer, int? Views, int? LiveAttends);
}