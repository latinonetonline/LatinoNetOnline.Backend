...
namespace LatinoNetOnline.Backend.Modules.Events.Core.Services
{
    ...
    class WebinarService : IWebinarService
    {
        ...
        public WebinarService(ApplicationDbContext dbContext, IMeetupService meetupService, IProposalService proposalService, IEventDispatcher eventDispatcher, IMessageBroker messageBroker)
        {
        ...
        {
            webinar.Status = WebinarStatus.Deleted;
        ...
        public async Task<OperationResult> UpdateWebinarNumbersAsync()
        {
            ...
            var lastWebinarNumberConfirmated = await _dbContext.Webinars
                .Where(x => x.Status == WebinarStatus.Published && x.StartDateTime.Date < DateTime.Today)
